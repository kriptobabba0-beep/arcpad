...
`BondingCurve.Completed` is **deliberately not** queried *by this watcher*, and the reason is worth knowing when you are reading a page: the watcher already reads `complete()` **from the slot, on every known curve, every poll**. For that one fact the slot is strictly better than the log — `complete` latches true and never goes back, so the log carries nothing newer. For the governance events the opposite holds, which is why those three *are* queried.

> **That argument is about CORRECTNESS and it stands. It does not answer LATENCY, and it used to read as though it did.** The slot says *whether* a curve is complete; it cannot say *when* it became complete, because a slot has no history. Only the log carries that. So the **executor** (`keeper/src/graduate/completedWatch.ts`, a separate process from this watcher) does query `Completed` — strictly as a **doorbell**: seeing one makes the next *ordinary* pass run early, and nothing is ever read out of the log. The graduation decision is still taken from `complete()`/`graduated()`, there is still exactly one execution path, and every gate (armed / quarantine / lock / simulate / read-back) is unchanged. The poll remains the backstop for dropped ranges, reorgs, and anything that completed while the executor was down. **This watcher is unchanged** and still does not query `Completed`.
>
> **The doorbell's cadence is measured, and "one block" is not available.** `KEEPER_GRADUATE_COMPLETED_WATCH_MS` defaults to **2000 ms**, not the ~350 ms one block would require. Measured 2026-08-10 against the live endpoint, sustained for 20 s: at **2000 ms**, 10 rounds, **zero refusals**; at **350 ms**, **30 of 47 requests refused** (`rate limit exceeded`, 64%). A per-block `eth_getLogs` is also ~40× the log-scan budget that was cut to 1 chunk/poll in §6 precisely because the keeper and the indexer starve each other. Set it to `0` to disable the doorbell entirely; latency then returns to the poll interval, which is what it was before, and nothing else changes.
...
|---|---|---|
| `watcher-heartbeat-missed` | No poll has run to completion for two poll intervals **and the log scan has reported no progress either** (budget 60 s). The process may be alive but wedged, or every poll is erroring. **A cold scan is not this** — see `watcher-catching-up`. | `pnpm --filter @arcpad/keeper start` logs — look for the immediately-preceding `PAGE` line naming the failing step. |
| `watcher-starting` (`OK`, not a page) | No poll has completed yet **and the FIRST one is still running**. A cold first poll reads the factory's slots before it scans anything, and under a rate limit that alone can exceed the 2-interval budget — measured: 2 of 4 cold starts paged `watcher-heartbeat-missed` for exactly this. The grace is anchored to the FIRST attempt and expires after 60 s, and it ends the moment a SECOND poll starts (which means the first one finished without succeeding). | Nothing for the first minute. If it persists, the watcher's first poll is wedged and the canary escalates on its own. |
| `watcher-catching-up` (`OK`, not a page) | No poll has completed, **but the log scan is advancing**: the watcher is alive and still walking `startBlock → head`. Against this factory a cold scan is 71 sequential `eth_getLogs` and **cannot** finish inside `2 × pollIntervalMs`, so this is the normal state after every restart until the cursor is warm. Exposure is not measured until the scan reaches the head. | Nothing. Watch `state=catching-up scanned=<block>/<head>` in the sink climb. If it stops climbing for 60 s the canary escalates to `watcher-heartbeat-missed`. |
| `chain-head-stale` | The RPC is serving a frozen view: the head block number was **the same across observations spanning** two poll intervals. Polls are "succeeding" and heartbeats are flowing — **only this detector sees it.** Because the window is computed from *chain* time, a frozen head means the window clock has stopped. **It measures observations, not wall clock**: a watcher that has simply not looked recently is `watcher-heartbeat-missed`, not this. (It used to measure wall clock, and paged "the RPC is serving a frozen view" while the head moved 71 blocks between two of its own messages.) | `curl -s -X POST $ARC_RPC_URL -H 'content-type: application/json' -d '{"jsonrpc":"2.0","id":1,"method":"eth_blockNumber","params":[]}'` twice, 30s apart. |
...
| `chain-time-frozen` | The chain timestamp was the same **across observations spanning** ten poll intervals. The budget is deliberately loose because **Arc documents that block timestamps may not increase**, so short pauses are normal and do not page. **It measures observations, not wall clock** — the same correction `chain-head-stale` got, which this detector did not get at the same time and which made it page falsely on a live cold start (reported "stuck at 1785938195" while chain time was 64 s past that). | Same checks as `chain-head-stale`. |
| `log-scan-failed` / `log-scan-incomplete` | The log scan threw on **two or more CONSECUTIVE polls** (so it is not converging), or the slot and the logs disagree about how many launches exist. It used to page whenever a poll completed zero chunks, which measured the CURSOR's backlog rather than the fault: once the watcher is caught up every poll scans one chunk, so every transient rate limit paged, while the identical fault during a cold backfill stayed silent. The window is still being classified correctly; **only the exposure number is untrustworthy.** | Read the direction in the page. `UNDER-reporting`: compare `launchCount()` on the explorer with the count in the page; the RPC's log index is behind or truncating. `OVER-reporting`: **stop the keeper, delete `keeper/.cursor`, restart.** The scan rebuilds from `startBlock`. Nothing is lost — the cursor is a cache, not a record. |
| `log-scan-catching-up` (`OK`, not a page) | The log scan stopped for this poll — normally Arc's rate limit mid-backfill — and this is the FIRST such poll in a row. It resumes from the persisted cursor next poll. Exposure is **not** measured this poll and the governance history is incomplete, which the line says. | Nothing, unless it repeats: the second consecutive failure pages as `log-scan-failed`. Progress is persisted **per chunk**, atomically (write-and-rename), so neither a `SIGTERM` nor a crash mid-write discards the walk. |
| `log-scan-deferred` (`OK`, not a page) | The scan hit its **per-poll chunk budget** (`KEEPER_LOG_SCAN_CHUNKS_PER_POLL`, default 1) and stopped on purpose. **The indexer owns the chain walk**; this watcher rations its own backfill. Measured: with both walking, the indexer completed ZERO ranges across two whole lives and exited on its rate-limit budget; with the keeper stopped it completed 25. | Nothing. Watch `scanned=<block>/<head>` climb, one chunk per poll. Raise the budget only if no indexer runs against this chain — and then expect to pay for it in the indexer's throughput if one is started later. |
| `cursor-reset` (`OK`, not a page) | The cursor file could not be used — wrong factory, wrong chain, unparseable (a crash mid-write leaves a truncated file). It is **ignored, not rejected**: the scan rebuilds from `startBlock` over the next few polls. | Nothing, unless it repeats every poll, which would mean something is corrupting the file continuously. |
...
**The in-process canary does not catch `SIGKILL`, and this is the one thing you must not assume it does.** A killed process emits nothing, including its own alarm. The mercy for that case is external: the alert sink receives a `HEARTBEAT keeper.graduationWindow ... state=<current|catching-up>` line — every poll when current, **and every scanned chunk while catching up** — and the sink's own "no heartbeat received in N minutes" rule is what fires. **Match the prefix, not the state**: a watcher mid-backfill is alive, and a rule that counted only `state=current` would declare every cold start dead — which is exactly what the old single-state heartbeat did. **Configure that rule when you configure the sink; a heartbeat nobody is counting is decoration.** The in-process canary covers *wedged but alive*, which is the more common and more insidious case.

> **The thing that counts them now exists, but only once you give it a URL.** `keeper/src/notify.ts` follows this sink and checks in to an external dead-man's switch on every heartbeat, of either state. It is deployed as `arcpad-keeper-notify@window.service` — see [`keeper-vps.md`](keeper-vps.md) §5, and note that the executor gets a **separate** switch, because one switch fed by both components would let the executor's check-ins keep it green while this watcher lay dead.
...
2. **`KEEPER_POLL_INTERVAL_MS=5000` used to produce a page storm, and no longer does.** With retries a poll costs 5.5–13 s of wall clock while the canary's budget is `2 × pollIntervalMs`, so at 5 s the old canary paged `watcher-heartbeat-missed` and `chain-head-stale` continuously *while the watcher was healthy*: **173 s at 5 s gave 22 heartbeats and 15 pages, every page a false positive**, and a cold start against the live factory gave **4 pages and 0 heartbeats in 40 s**. Both were the same defect — liveness was tied to *poll completion*, and a legitimate long scan cannot complete inside the budget. Liveness is now tied to *scan progress*, and the head detector measures observations rather than wall clock. **Re-measured on 2026-08-05, cold cursor, live factory, `KEEPER_POLL_INTERVAL_MS=5000`: 65 s gave 0 pages, 15 `state=catching-up` heartbeats and 2 `OK` notices.**

**THE INDEXER OWNS THE CHAIN WALK, AND THAT IS A DECISION, NOT AN ACCIDENT.** The keeper and the
indexer were both walking `eth_getLogs` over the same factory from the same start block, neither
aware of the other. Measured: with the keeper cold-scanning, two supervised indexer lives completed
**zero** 10,000-block ranges and both exited on their rate-limit budget; with the keeper stopped,
the same binary against the same endpoint completed 25. The keeper now takes at most
`KEEPER_LOG_SCAN_CHUNKS_PER_POLL` chunks per poll (default **1**, i.e. ~0.2 `eth_getLogs`/s at a
5 s interval), which has a second and larger benefit: `readWindowState` + `classify` + the paging
path now run **every poll** instead of queueing behind a multi-minute scan, so the detection latency
for a hostile proposal during a backfill drops from minutes to one poll interval. The cost is that
the keeper's own backfill takes ~77 polls (~6.5 min) on this deployment — and it says so, every
poll, as `state=catching-up` plus `log-scan-deferred`.
...
>
> **Updated again 2026-08-09 — the count is still four, and the reason one of them survives has changed.** *Paging provider* used to mean "nothing reads these lines and nothing is written that would"; the reading half is now built and tested (`keeper/src/notify.ts`), so what remains is a provider choice and two URLs. *Drill transport* is unchanged in substance but is now actionable, because a VPS has been chosen. **Neither was removed**: a hole whose mechanism exists is still a hole while its value is blank, and recording it as closed because the code landed would be the exact "mechanism exists but its output goes nowhere" shape this file is full of. Deployment artifacts, and the six holes the VPS itself carries, are in [`keeper-vps.md`](keeper-vps.md).
>
...
| **Paging provider** | **`TODO(owner)`** — which provider, and the two rules it must have: (1) any line starting with `PAGE ` pages the on-call; (2) **no line starting with `HEARTBEAT ` for 10 minutes pages the on-call.** Rule (2) is the dead-man's switch and it is the only thing that catches a killed process — see §6. A heartbeat nobody counts is decoration. **Both rules are now implemented in code** (`keeper/src/notify.ts`, run as `arcpad-keeper-notify@.service`): it follows the sink, POSTs every `PAGE` record to one URL and checks in to a dead-man's switch on every `HEARTBEAT`, and refuses to start with only one of the two configured. So this hole is no longer "build the mechanism" — it is **choose a provider and paste the URLs**. Any endpoint that accepts an HTTP POST works, and the choice changes no code. See [`keeper-vps.md`](keeper-vps.md) §5. |
| **Drill transport** | **`TODO(owner)`** — publish `$KEEPER_ALERT_LOG` at a URL the CI drill can `curl`, and set the repository variable `KEEPER_ALERT_LOG_URL`. Until this is set the weekly `observe` job fails with that exact message rather than passing on an empty file. **A VPS is where this becomes fillable**; the recipe and the two ways to get it wrong are in [`keeper-vps.md`](keeper-vps.md) §7. |
...
2. **at least one `HEARTBEAT`, of either state.** `state=catching-up` counts: it proves the watcher was alive. The drill's own output says so when *every* beat was `catching-up`, because that also means exposure was never measured in the window.
