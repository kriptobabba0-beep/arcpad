...
|---|---|---|---|
| `graduationTarget` | `proposeGraduationTarget` (governor) → `applyGraduationTarget` (**izinsiz**) | **3 gün**, kamuya açık | Hedef bir launch'ın **tüm** raise'ini alır, ve `graduate()` çağrı anında okuduğu için bir yeniden işaretleme zaten tamamlanmış curve'lerin ödemesini de yeni adrese yönlendirir. Üç günlük pencerede herkes o curve'leri mevcut hedefe boşaltabilir. |
