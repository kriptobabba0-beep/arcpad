...
CREATE TABLE buyback_events (
  ...
  --
  -- `policy` OTEKI BESTEN FARKLI BIR SINIFTIR: bir tutar degil bir KARAR
  -- tasir (`LaunchFactory.BuybackEnabledUpdated`). Ayni tabloda durmasinin
  -- sebebi, "para nerede" sorusunun cevabinin politikayi ICERMESI: bir
  -- tokende tahakkukun DURMASI bir ariza degil, creator'in ozelligi kapatmis
  -- olmasi olabilir, ve iki tabloya bolunmus bir gecmis o cumleyi kurmak icin
  -- her seferinde bir UNION isterdi. Kapsam kontrolu de ayni defteri arar.
  kind          text NOT NULL
                CHECK (kind IN ('policy','accrued','executed','skipped','locked','released')),
  ...
  -- `released`te `release()`i cagiran, `policy`de degisikligi YAPAN.
  --
  -- IKI TURDE DE AYNI SORUYU CEVAPLAR ("kim yapti") ve bu yuzden TEK kolon:
  -- ayri bir `by_addr` acmak, ayni soruyu iki adla sormak olurdu. Politika
  -- tarafinda cevap ayrica bir YETKI kaydidir -- creator mi acti, governor mu
  -- kapatti -- ve o ayrim izin modelinin kendisidir.
  caller_addr   text CHECK (caller_addr ~ '^0x[0-9a-f]{40}$'),
  -- YALNIZCA `policy`: yeni deger. `true` = acildi, `false` = kapatildi.
  --
  -- Bir GECIS degil bir DURUM saklanir, cunku zincir de oyle yayar
  -- (`setBuybackEnabled` ayni degere ayarlamayi SESSIZCE gecer, yani her log
  -- gercek bir degisimdir ve yeni deger her zaman oncekinin tersidir).
  enabled       boolean,
  ...
  -- `caller_addr` IKI turde dolu, DIGER DORTTE bos. Tek yonlu yazmak
  -- (`policy => dolu`) bir tahakkuk satirinda bir cuzdan adresinin durmasina
  -- izin verirdi ve okuma tarafi onu "bunu biri yapti" diye gosterirdi.
  CONSTRAINT buyback_caller_iff_actor_kind CHECK (
    (kind IN ('released','policy')) = (caller_addr IS NOT NULL)),
  CONSTRAINT buyback_enabled_iff_policy CHECK (
    (kind = 'policy') = (enabled IS NOT NULL)),
...
CREATE TABLE buyback_state (
  ...
  -- ============ ARAYUZUN ILK SORDUGU SEY ============
  --
  -- "Bu token'da buyback acik mi." Asagidaki toplamlardan TURETILEMEZ ve
  -- turetmeye calismak tam olarak yanlis cevabi verir: ozelligi acmis ama
  -- henuz islem gormemis bir token'in butun toplamlari sifirdir, yani
  -- `accrued_total_wei > 0` yuklemi ona "kapali" der. Kapatma yonu ise hic
  -- gorunmez -- gecmis tahakkuklar yerinde kalir.
  --
  -- VARSAYILAN `false` VE BU DOGRUDUR: `LaunchFactory.buybackEnabledOf` da
  -- oyle baslar, ve satir zaten yalnizca bir buyback olayi geldiginde olusur.
  enabled               boolean NOT NULL DEFAULT false,
  /* Politikayi EN SON degistiren olay ve onu yapan. `enabled`in yaninda
     yasarlar cunku ekranda "creator 3 gun once kapatti" cumlesi ucunu
     birden ister. */
  enabled_seq           bigint,
  enabled_by_addr       text CHECK (enabled_by_addr ~ '^0x[0-9a-f]{40}$'),
  ...
  CONSTRAINT buyback_state_window_is_forward CHECK (
    vesting_end_at IS NULL OR vesting_end_at > vesting_start_at),
  -- Politikanin uc alani BIRLIKTE yasar: ya hic politika olayi gorulmedi
  -- (ikisi de NULL, `enabled` varsayilan `false`), ya da gorulduyse HANGI olay
  -- ve KIM oldugu bilinir. `graduated_iff_payout`un aynisi.
  CONSTRAINT buyback_policy_provenance CHECK (
    (enabled_seq IS NULL) = (enabled_by_addr IS NULL))
);

-- ---------------------------------------------------------------------------
-- OKUMA MODELI: `token_overview` 009'un tanimini SUPERSEDE EDER.
--
-- NICIN BIR LISTE SUTUNU GEREKIYOR. Buyback bir CREATOR TAAHHUDUDUR ve isini
-- ancak GORULEBILDIGI kadar yapar: yalnizca token sayfasinda duran bir rozet,
-- gezinen bir kullaniciya hangi tokenlerin bu taahhudu tasidigini SOYLEMEZ.
-- Alternatif -- kart basina bir sorgu ya da bir `eth_call` -- kirk kartlik bir
-- izgarada kirk yuvarlak yol demekti.
--
-- IKI SUTUN, VE IKISI DE `LEFT JOIN`DEN: buyback gormemis bir token'in
-- `buyback_state` satiri YOKTUR, ve `coalesce` onu `false`/`0`a indirir --
-- yani "hic olmadi" ile "kapatildi" LISTEDE ayni gorunur. Bu bilinclidir:
-- ayrim TOKEN SAYFASINA aittir (orada `getTokenBuyback` `null` doner) ve bir
-- kart rozetinin tasiyabilecegi bir nuans degildir.
--
-- `::numeric(78,0)` CASTI ZORUNLUDUR, suslemek icin degil: `coalesce(x, 0)`
-- typmod'u DUSURUR ve adlandirma kapisi her `_tok` sutununun TAM OLARAK
-- numeric(78,0) olmasini ister -- dogru olarak.
-- ---------------------------------------------------------------------------
...
SELECT
  ...
  -- CREATOR TAAHHUDU. `graduated`in yanina konuyor cunku ayni soruyu farkli
  -- bir eksende cevaplar: "bu launch neye BAGLANDI".
...
-- `LEFT JOIN`: buyback gormemis bir token de listede DURMALI. `JOIN` yazmak,
-- ozelligi acmamis butun tokenleri explore sayfasindan SILERDI.
