...
--
--   nearGraduation  Sort=2 -- COZULEBILIR, ve tasarimi belirlendi.
--                             `progress_ppm` `curve_state` x `deployment`ten
--                             hesaplanir; `_ppm` zaten bildirilmis bir sonek
--                             (`naming.test.ts`), `ts.created_seq` zaten var,
--                             ve bakim noktasi `applyTrade`in MEVCUT `st`
--                             CTE'sidir -- AYRI bir CTE DEGIL, bkz. asagi.
--
--   marketCap       Sort=2 -- BIR URUN KARARI BEKLIYOR, performans isi degil.
--
-- MARKETCAP NEDEN BASKA BIR SEY: iki deger BAYATLIKTAN degil TASARIMDAN ayrisir.
...
-- View ise `market_cap_wei`i `curve_state`ten hesaplar ve egri mezuniyette
-- DONAR. Yani mezun bir token icin:
--
--   ts.market_cap_wei          -> havuzun CANLI fiyati
--   view'in market_cap_wei'i   -> egrinin DONMUS son fiyati
--
-- View'i `ts`e cevirmek bu yuzden bir indeks degisikligi degil, mezun
-- tokenlerin GOSTERILEN market cap'ini ve `marketCap` siralamasinin ANLAMINI
-- degistirmek olurdu. (Attempt 2'de dusen dokuz test bugunku anlami kodluyor.)
-- Karar sahibinin secmesi gereken sey:
--   A) view egri turevli kalir -> indeks, egri ifadesini AYNEN yansitan (ve
--      mezuniyette donan) ayri bir sakli sutun ister;
--   B) view `ts`e gecer -> mezunlar canli havuz market cap'i gosterir, ki bu
--      muhtemelen kullanici icin DAHA dogru, ama gorunen degerleri degistirir.
--
-- POSTGRES SINIRI, VE NEDEN "AYRI BIR CTE EKLE" YANLIS OLURDU: tek bir ifadede
-- AYNI satiri iki kez guncellemek desteklenmez -- ikinci etki sessizce KAYBOLUR.
-- `applyTrade` `token_stats`i zaten `st` CTE'sinde guncelliyor, dolayisiyla
-- `progress_ppm` (ve secilirse market cap) O CTE'nin SET listesine katilmalidir.
-- Ayri bir `mc AS (UPDATE token_stats ...)` yazmak ya hacim sayaclarini ya da
-- yeni sutunu dusururdu, ve hangisini dusurdugu sessiz olurdu.
--
...
-- ------------------------------------------------------------------
-- 2. VIEW: TEK SUTUNUN KAYNAGI DEGISIR
-- ------------------------------------------------------------------
...
-- cikarilir, yalnizca BIRININ kaynagi degisir. `ts.created_seq` da `bigint`,
-- yani tip AYNI.
--
-- VE DEGER DE AYNI -- BU YAPISAL, TESADUFI DEGIL:
--   * `applyLaunch` ikisini TEK ifadede, AYNI `ins` CTE'sinden yazar
--     (`INSERT INTO token_stats ... SELECT token, 0, 0, created_seq, created_at
--     FROM ins`), yani ayni degeri iki tabloya koyar;
--   * hicbir `UPDATE` `created_seq`e dokunmaz -- ne bu paket, ne indexer;
--   * `JOIN token_stats` bir INNER JOIN, yani NULL riski ve satir kumesi
--     degisikligi de yok.
-- Ve `applyLaunch` TEK yazma yolu: indexer de (`admit.ts`) onu cagirir.
-- `test/sort-keys.test.ts` bu esitligi ayrica KAPI haline getirir.
